...
class ParticipantModel extends Model
{
    ...
    public function getDeltagerFieldsWithNames()
    {
        ...
        foreach($field_keys as $key) {
            $name = isset($field_names[$key]) ? $field_names[$key] : $key;
            $fields[$key] = $name;
        }
        return $fields;
    }
        ...
        foreach($columns as $name => $type ) {
            switch(true) {
                ...
                case $name == "brugerkategori_id": // Avoid error on foreign key constraint 
                    $fields[$name] = 1;
                    break;
                case $name == "dancing_with_the_clans": // Very long enum 
                    $fields[$name] = 'nej';
                    break;
                ...
                case $type == "enum('ja','nej')":
                    $fields[$name] = 'nej';
                    break;
                case $type == "enum('m','k','a')":
                    $fields[$name] = 'a';
                    break;
                case strpos($type, "set(") !== false || strpos($type, "char(") !== false || $type == "text":
                    $fields[$name] = '';
                    break;
                case $type == "datetime";
