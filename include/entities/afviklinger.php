...
';
  ...
  public function createVotes(Page $page)
  {
    ...
    // Load existing vote entries
    $query = 'SELECT * FROM schedules_votes WHERE schedule_id = ? ORDER BY id';
    $votes = $this->db->query($query, [$this->id]);

    $activity = $this->getActivity();
    $teams    = $this->getHold();
    $missing_count = ($activity->max_deltagere_per_hold + $activity->spilledere_per_hold) * count($teams) - count($votes);
    
    // We don't need to add any codes
    if ($missing_count < 1) return $votes;

    // Generate new codes
    $codes = [];
    for ($i = 0; $i < $missing_count; $i++) {
      $codes[] = $this->makeVoteCode();
    }

    // Insert new votes
    $query  = "INSERT INTO schedules_votes (schedule_id, code, cast_at) VALUES ($this->id, ?, '0000-00-00 00:00:00')";
    $query .= str_repeat(", ($this->id, ?, '0000-00-00 00:00:00')", $missing_count - 1);
    $this->db->exec($query, $codes);

    // Load votes again to get the ID of the new votes
    $query = 'SELECT * FROM schedules_votes WHERE schedule_id = ? ORDER BY id';
    $votes = $this->db->query($query, [$this->id]);

    // Check we have a directory for QR codes
    ...
    foreach ($votes as $vote) {
      $QRcode_file = $output_dir . $vote['id'] . '.png';
      if (!file_exists($QRcode_file)) {
        QRcode::png($page->url('activity_specific_vote', ['code' => $vote['code']]), $QRcode_file, 'M', 3);
      }
    }
